...
## 2026-05-31 - Visual Feedback for Async Buttons
**Learning:** Adding `aria-busy={true}` to buttons undergoing async operations is great for screen readers, but sighted users also need a visual indicator (like a spinner) to ensure sensory parity and prevent double submissions.
**Action:** Always pair `aria-busy={true}` with an animated visual indicator, such as an SVG spinner with `aria-hidden="true"`.
