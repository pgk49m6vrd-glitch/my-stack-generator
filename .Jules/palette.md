...
## 2025-04-16 - Semantic Navigation Lists
**Learning:** Using `<div>` wrappers and decorative characters (like `|`) for inline navigation creates clutter for screen readers and breaks semantic structure.
**Action:** Always use `<nav>` with `<ul>`/`<li>` elements and CSS layout spacing (`gap`) for groups of links to ensure semantic, accessible navigation.
