...
**Learning:** Adding aria-busy={true} to a button communicates the loading state to screen readers, but sighted users need a visual indicator like an animated spinner to understand the state change, creating sensory parity.
**Action:** Always pair `aria-busy={true}` with a visual indicator, such as an animated SVG spinner with `aria-hidden="true"`, on async action buttons.
