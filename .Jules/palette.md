...
## 2025-06-06 - Sensory Parity in Loading States
**Learning:** Using `aria-busy` alone makes loading invisible to sighted users, while visual spinners without ARIA are invisible to screen readers.
**Action:** Always pair `aria-busy={true}` with a visual indicator (like an SVG spinner with `aria-hidden="true"`) for interactive elements.
