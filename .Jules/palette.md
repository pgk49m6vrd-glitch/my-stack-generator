...
## 2025-05-23 - Semantic Navigation Lists
**Learning:** Using generic `<div>` wrappers and unsemantic text characters like `|` to separate inline navigation links clutters screen reader output and reduces accessibility.
**Action:** When separating inline navigation links or items in generated UI templates, use semantic `<nav>` and `<ul>`/`<li>` elements combined with CSS layout spacing (e.g., flex gaps) to improve accessibility, responsiveness, and reduce screen reader clutter.
