...
## 2026-03-03 - Semantic Navigation for Accessibility
**Learning:** Using generic `<div>` wrappers and unsemantic decorative characters (like `|`) for inline navigation links creates a poor experience for screen reader users and hurts accessibility.
**Action:** When separating inline navigation links in generated UI templates, always use semantic `<nav>` and `<ul>`/`<li>` elements combined with CSS layout spacing instead of decorative text characters.
