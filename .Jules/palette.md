...
## 2025-05-23 - Semantic Navigation for Link Groups
**Learning:** Using generic `div`s with inline characters (like `|`) to separate links creates clutter for screen readers and breaks responsiveness.
**Action:** Always use semantic `<nav aria-label="description">` combined with `<ul>`/`<li>` wrappers and CSS `gap` properties (like Tailwind's `flex gap-4`) for navigation links, improving accessibility, responsive design, and reducing cognitive load.
