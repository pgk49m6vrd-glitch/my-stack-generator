...
## 2026-03-12 - Semantic Navigation for Documentation Links
**Learning:** Separating inline navigation links or items using generic `<div>` wrappers and unsemantic decorative text characters (e.g., `|`) causes screen reader clutter and breaks accessibility.
**Action:** Use semantic `<nav>` and `<ul>`/`<li>` elements combined with CSS layout spacing instead.
