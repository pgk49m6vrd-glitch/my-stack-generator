...
## 2025-03-23 - Improve inline navigation semantics and accessibility
**Learning:** Using generic `<div>` wrappers and unsemantic decorative text characters (like `|`) for inline navigation creates screen reader clutter and reduces accessibility.
**Action:** Use semantic `<nav aria-label="Documentation links">` and `<ul>`/`<li>` elements combined with CSS layout spacing (e.g., `flex gap-4`) to separate inline navigation links instead.
