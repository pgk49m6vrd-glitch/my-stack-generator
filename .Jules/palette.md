...
## 2026-03-03 - Semantic Navigation for Link Lists
**Learning:** Using generic `<div>` wrappers and decorative text characters (like `|`) to separate inline links creates screen reader clutter and lacks structural meaning for assistive technologies.
**Action:** When separating inline navigation links or items in generated UI templates, use semantic `<nav aria-label="...">` and `<ul>`/`<li>` elements combined with CSS layout spacing (like Tailwind's `flex gap-4` or `flex-wrap`) instead of unsemantic wrappers and text separators.
