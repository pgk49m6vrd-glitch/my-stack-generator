...
## 2026-04-13 - Semantic Navigation Elements
**Learning:** Using generic `<div>` wrappers and text characters like `|` for inline navigation links clutters the screen reader experience and reduces semantic meaning.
**Action:** Use semantic `<nav>` and `<ul>`/`<li>` elements for navigation groups, relying on CSS for layout (e.g., `flex gap-4`) instead of decorative text separators.
