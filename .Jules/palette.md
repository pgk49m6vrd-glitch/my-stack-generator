...
## 2025-05-24 - Semantic Navigation Lists
**Learning:** Using generic `<div>` wrappers and unsemantic text characters (like `|`) for navigation spacing creates screen reader clutter and breaks semantic structure.
**Action:** When separating inline navigation links, use semantic `<nav>` and `<ul>`/`<li>` elements combined with CSS layout spacing (`gap`) to improve accessibility and structure.
