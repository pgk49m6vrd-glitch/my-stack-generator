...
## 2026-05-25 - Synchronizing Visual and Assistive Async States
**Learning:** Native `aria-busy` and text changes (like "Loading...") provide robust accessibility feedback, but lack visual weight. This creates cognitive dissonance for sighted users who expect an animated visual indicator during async actions.
**Action:** Pair `aria-busy={true}` with an animated SVG spinner (`aria-hidden="true"`) to ensure visual and assistive sensory parity.
