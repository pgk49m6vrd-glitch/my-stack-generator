...
## 2026-06-03 - Loading States Need Sensory Parity
**Learning:** Adding `aria-busy={true}` to buttons undergoing async operations fulfills screen reader requirements but leaves sighted users with only text changes, breaking sensory parity and visual feedback expectations.
**Action:** Always pair `aria-busy={true}` with a visual indicator, such as an animated SVG spinner with `aria-hidden="true"`.
