...
## 2026-06-07 - Sensory Parity for Loading States
**Learning:** Adding `aria-busy={true}` to a loading button alerts screen readers, but sighted users need equivalent visual feedback beyond just text changing.
**Action:** Always pair `aria-busy={true}` with a visual indicator, such as an animated SVG spinner with `aria-hidden="true"`, to ensure sensory parity.
