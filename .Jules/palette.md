...
## 2026-06-29 - Linking Form-Level Errors
**Learning:** Even when error messages are displayed at the form level (e.g., above the form), they are invisible to screen readers at the input level unless explicitly linked.
**Action:** Always ensure error containers have an `id` and are explicitly associated with relevant input fields using `aria-invalid` and `aria-describedby` attributes to maintain accessibility.
