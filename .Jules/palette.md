...
## 2026-03-03 - Semantic Navigation Lists
**Learning:** Using generic `<div>` wrappers and unsemantic text characters (like `|`) as visual separators in navigation links creates clutter for screen readers and breaks semantic structure.
**Action:** When separating inline navigation links or items, always use semantic `<nav>` and `<ul>`/`<li>` elements combined with CSS layout spacing (like Tailwind's `flex gap-4`) instead of decorative text characters.
