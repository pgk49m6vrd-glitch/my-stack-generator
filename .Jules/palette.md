...
## 2026-03-02 - Semantic Lists vs Text Separators
**Learning:** Separating inline navigation links or items using generic `<div>` wrappers and unsemantic decorative text characters (e.g., `|`) clutters the screen reader experience and reduces responsiveness.
**Action:** Use semantic `<nav>` and `<ul>`/`<li>` elements combined with CSS layout spacing (like Tailwind's `flex gap-4`) to improve accessibility, responsiveness, and reduce screen reader noise.
