...
## 2025-05-23 - Visual Loading Indicators
**Learning:** Adding `aria-busy` is not enough. Sighted users need visual feedback when waiting for async operations.
**Action:** Pair `aria-busy={true}` with a visual indicator, such as an animated SVG spinner with `aria-hidden="true"`, to ensure sensory parity between sighted users and screen readers.
