...
## 2026-03-01 - Parallelize Package Manager Checks
**Learning:** Sequential execution of `await checkPackageManager` for 'npm', 'pnpm', and 'bun' within the project generator CLI delays the initial user prompt rendering by executing separate subprocesses or environment checks sequentially.
**Action:** Use `Promise.all` to resolve these checks concurrently, removing sequential blocking and allowing faster rendering of the package manager selection options.
