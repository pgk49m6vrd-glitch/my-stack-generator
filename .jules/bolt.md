...
## 2024-11-20 - Map Has/Get Pattern Optimization
**Learning:** In hot paths (like checking `pmAvailability` or `templateCache`), using `Map.has(key)` followed by `Map.get(key)` causes two sequential hash lookups.
**Action:** Replace `if (Map.has(key)) { return Map.get(key) }` with `const val = Map.get(key); if (val !== undefined) { return val; }` to reduce hash lookups and improve performance in high-frequency functions.
