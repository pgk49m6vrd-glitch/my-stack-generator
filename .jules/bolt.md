...
## 2024-05-23 - CLI Top-level Dependency Loading
**Learning:** Loading heavy dependencies or subcommand modules at the top level of a CLI entry point (e.g., `src/cli.js`) causes them to be parsed and evaluated on every invocation, including lightweight commands like `--help`.
**Action:** Delay loading of subcommand modules by dynamically importing them (`await import()`) inside the command's action handler. This avoids the synchronous top-level load penalty.
