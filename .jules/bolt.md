...
## 2026-05-03 - Lazy Load CLI Subcommands
**Learning:** Top-level static imports for subcommands (like initCommand, listPresets) pull in heavy dependencies synchronously, causing a significant startup delay for simple commands like `--help`.
**Action:** Use dynamic `await import()` within command action handlers to delay dependency loading until the command is actually executed, significantly reducing CLI response time.
