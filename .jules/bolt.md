...
## 2024-06-08 - Cosmiconfig Initialization Overhead
**Learning:** In Node.js applications, repeatedly instantiating `cosmiconfig()` inside a frequently called function introduces measurable initialization overhead (~13ms per call) because it re-initializes caches and filesystem abstractions.
**Action:** Cache the explorer instance at the module scope to reuse it across calls, which significantly reduces the execution time of repeated configurations searches.
