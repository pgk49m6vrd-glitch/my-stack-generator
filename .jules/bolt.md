...
## 2024-05-22 - CLI Subcommand Dynamic Imports
**Learning:** Static top-level imports of CLI subcommands that depend on heavy libraries (like `cosmiconfig`) significantly increase the startup time of the entire CLI, even for simple commands like `--help`.
**Action:** Switch to dynamic imports (`await import()`) within the specific command action handlers. This defers the loading penalty to when the command is actually executed, significantly reducing startup overhead.
