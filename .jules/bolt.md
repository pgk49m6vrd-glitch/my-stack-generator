...
## 2026-02-12 - Async Package Manager Checks
**Learning:** Checking for package manager availability synchronously (via `spawn.sync`) blocks the main thread, causing noticeable delays (1-2s) in CLI responsiveness during selection steps.
**Action:** Initiate asynchronous checks (`spawn` + Promise) at startup and await the result when needed. This parallelizes the IO with user interaction, making the CLI feel instant.
