...
## 2024-05-18 - CLI Startup Time Bottlenecks
**Learning:** Top-level synchronous static imports of complex logic or large dependencies in a Node.js CLI can significantly slow down basic boot tasks like `--help` or `--version`, because the module tree must be fully parsed before execution.
**Action:** Use dynamic imports (`await import()`) inside specific command action handlers to ensure that expensive modules are only evaluated when their respective commands are actually invoked.
