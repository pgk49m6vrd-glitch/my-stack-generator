...
## 2024-05-26 - Async File Read Optimization
**Learning:** To optimize performance in asynchronous functions that read files, avoid redundant synchronous `fs.existsSync` checks. Instead, directly call `fs.promises.readFile` and handle the potential missing file case (`ENOENT`) within a `try-catch` block to prevent blocking the event loop. In Node.js performance optimization, replacing `fs.existsSync()` with a `try-catch` block around `fs.readFileSync()` actually degrades performance significantly on cache misses due to the overhead of generating V8 Error objects. The `try-catch` pattern should be reserved exclusively for asynchronous `fs.promises.readFile()`.
**Action:** Use `try-catch` around `fs.promises.readFile()` instead of `fs.existsSync()` to avoid event-loop blocking, and keep `fs.existsSync()` for synchronous reads to prevent V8 Error generation overhead.
