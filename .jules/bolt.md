...
## 2024-05-31 - Asynchronous File Reads
**Learning:** Using synchronous `fs.existsSync` inside an asynchronous file read flow blocks the Node.js event loop unnecessarily.
**Action:** Replace `fs.existsSync()` with a direct `fs.promises.readFile()` call wrapped in a `try-catch` block to handle cache misses (`ENOENT`), keeping the operation fully non-blocking.
