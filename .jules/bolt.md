...
## 2026-02-13 - Optimizing Interactive CLI Latency
**Learning:** Pre-computing external command checks (like `npm --version`) in the background while the user is typing can significantly improve perceived performance in interactive CLIs.
**Action:** Use "fire-and-forget" async checks with result caching for any environment validation steps that don't depend on user input, but ensure to handle cache invalidation for retry scenarios.
