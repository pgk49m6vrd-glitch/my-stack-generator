...
## 2024-06-05 - Cosmiconfig Initialization Overhead
**Learning:** Instantiating `cosmiconfig('mystack')` on every call to load configuration incurs a measurable startup penalty (~13ms per call) because it re-initializes caches and filesystem abstractions.
**Action:** Cache the explorer instance at the module scope so that subsequent calls within the same process reuse the instance, saving time and fully utilizing its internal cache.
