...
## 2024-05-23 - CLI Dynamic Imports
**Learning:** Static top-level imports of heavy dependencies like `cosmiconfig` and subcommand modules (`initCommand`, `listPresets`) introduce synchronous top-level load penalties, increasing the CLI startup time significantly.
**Action:** Use dynamic imports (`await import()`) inside the specific execution paths (e.g., action handlers) to lazy-load these modules only when they are triggered.
