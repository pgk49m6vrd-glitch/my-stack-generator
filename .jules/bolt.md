...
## 2026-04-20 - Dynamic imports for CLI performance
**Learning:** To maintain CLI startup performance, heavy dependencies like `cosmiconfig` and subcommand modules (e.g., `initCommand`, `listPresets`) must be dynamically imported (`await import())` only when their specific execution paths are triggered, avoiding synchronous top-level load penalties.
**Action:** Use dynamic imports for CLI command handlers to reduce cold start times.
