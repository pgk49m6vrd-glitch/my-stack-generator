...
## 2024-05-22 - Dynamic Imports for CLI Subcommands
**Learning:** Synchronously importing heavy subcommand modules (and their dependencies) at the top of the CLI entry point significantly slows down startup time, especially when just viewing help or running unrelated commands.
**Action:** Use dynamic imports (`await import()`) within the specific command's action handler to defer loading until the command is actually executed. This reduced CLI startup time from ~145ms to ~67ms.
