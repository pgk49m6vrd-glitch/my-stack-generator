...
## 2025-05-30 - CLI Feature Allowlist Bypass
**Vulnerability:** Unvalidated `--features` CLI flag in non-interactive mode.
**Learning:** Array lists or comma-separated lists passed via CLI are just as susceptible to injection and out-of-bounds inputs as single values. Even if only checked via `.includes()`, failing to validate against an allowlist violates secure-by-default principles and risks unexpected downstream behavior.
**Prevention:** Always validate every item in array-based CLI inputs against a strict allowlist.
