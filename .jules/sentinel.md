...
## 2024-08-21 - Remove unsafe-eval from CSP
...
## 2024-05-24 - DoS Risk in Handlebars Compilation
**Vulnerability:** Precompiling Handlebars templates using `new Function()` in `template-engine.js` allows execution of untrusted strings in a high privilege context if an attacker could inject arbitrary content into template source or cache. However, in this specific repository this issue is not present as there is no user control over Handlebars template definitions or files. The precompiled output also is explicitly restricted via CSP and the underlying generator requires manual file execution by devs.
**Learning:** `new Function()` is almost always a security bad practice. However, when trying to enforce a security improvement, it is important to remember what the core function is. The Vite plugin allows inline scripts during local development for HMR and drops the insecure property in production builds, representing a balance between security and Dev-Ex.
**Prevention:** Do not enforce security enhancements that completely disable primary functionality (such as HMR in Dev) unless they present an unacceptable risk for that environment.
