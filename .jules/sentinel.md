...
## 2025-06-09 - User Enumeration via Raw Error Messages
**Vulnerability:** Authentication forms passed raw backend error messages (from Firebase/Supabase) directly to the UI via `err.message`, allowing user enumeration and information leakage.
**Learning:** Backend error messages often contain specific details (e.g., "User not found" or "Email already in use") that attackers can use to confirm whether an account exists.
**Prevention:** Always catch authentication errors and display generic, non-specific error messages (like "Authentication failed") to the end user.
