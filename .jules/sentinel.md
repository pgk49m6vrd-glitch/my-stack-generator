...
**Learning:** CLI parameters that are later passed to OS-level child processes or used as object keys must be rigorously validated even when reasonable defaults are provided. In non-interactive modes, validation logic is often accidentally bypassed.
**Prevention:** Always implement strict allowlists for sensitive CLI arguments before any process execution, failing securely (e.g., `process.exit(1)`) if inputs are out-of-bounds. Safely handle `undefined` cases to preserve legitimate defaults.
