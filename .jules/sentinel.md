...
## 2024-05-18 - [Command Injection via Unvalidated CLI Input Allowlist Expansion]
**Vulnerability:** The CLI accepted any string for the features flag (`--features`) without validation, allowing invalid or potentially malicious flags to pass through.
**Learning:** All CLI inputs, especially those defining array or comma-separated lists like features, must be strictly validated against an allowlist to prevent unknown configurations or potential injection vectors.
**Prevention:** Always implement strict allowlists for array-based or comma-separated CLI inputs, verifying each item against the allowlist before further processing.
