...
## 2024-05-20 - [Array-Based CLI Input Allowlist]
**Vulnerability:** Unvalidated array-based inputs (like comma-separated features) present injection risks and logical bugs.
**Learning:** Checking a comma-separated string isn't enough; after splitting, each individual array element must be validated against a strict allowlist.
**Prevention:** Always validate every item in array-based CLI inputs against a strict allowlist to prevent injection or out-of-bounds inputs. Safely handle undefined or non-array inputs.
