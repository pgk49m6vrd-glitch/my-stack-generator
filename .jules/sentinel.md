...
## 2024-05-18 - [Command Injection via Unvalidated CLI Input (Features Allowlist)]
**Vulnerability:** The CLI accepted any string for the features flag (`--features`) in non-interactive mode. While currently used to select templates, unvalidated array-like inputs can be abused if passed to other systems or child processes.
**Learning:** All CLI parameters, even those that seem harmless (like a comma-separated list of features), must be rigorously validated against a strict allowlist.
**Prevention:** Always implement strict allowlists for array-based or comma-separated CLI inputs, failing securely (e.g., `process.exit(1)`) if inputs are out-of-bounds, and safely handle `undefined` or non-array inputs.
