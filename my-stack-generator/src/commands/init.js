...
export async function initCommand(options = {}) {
  ...
  if (options.yes) {
    ...
    // Validate features to prevent command injection or out-of-bounds inputs
    if (Array.isArray(config.features)) {
      const invalidFeatures = config.features.filter(f => !ALLOWED_FEATURES.includes(f));
      if (invalidFeatures.length > 0) {
        console.error(`\n❌ Security Error: Unsupported features detected "${invalidFeatures.join(', ')}". Allowed: ${ALLOWED_FEATURES.join(', ')}`);
        process.exit(1);
      }
    }
