...
export async function initCommand(options = {}) {
  ...
  if (options.yes) {
    ...
    // Security: Validate package manager and backend to prevent command injection
    const allowedPMs = ['npm', 'pnpm', 'bun'];
    if (config.pm !== undefined && !allowedPMs.includes(config.pm)) {
      console.error(`\n❌ Error: Invalid package manager "${config.pm}". Allowed: ${allowedPMs.join(', ')}`);
      process.exit(1);
    }
    const allowedBackends = ['firebase', 'supabase'];
    if (config.backend !== undefined && !allowedBackends.includes(config.backend)) {
      console.error(`\n❌ Error: Invalid backend "${config.backend}". Allowed: ${allowedBackends.join(', ')}`);
      process.exit(1);
    }
