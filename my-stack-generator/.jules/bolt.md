...
## 2024-05-24 - RegEx Optimization in Validation
**Learning:** Checking for reserved names using a complex regex before verifying basic character constraints causes a slight performance penalty, as basic invalid strings are unnecessarily tested against the more complex regex.
**Action:** Reorder regex validation steps to evaluate the most basic/restrictive character regex (`VALID_NAME_REGEX`) before more complex rule-based tests (`RESERVED_NAMES`), allowing an earlier return for fundamentally invalid strings.
