...
async function sanitizePackageName(name) {
  ...
  // Lazy load to improve CLI startup time (Time To First Prompt)
...
export function checkPackageManager(pm) {
  ...
  const checkPromise = new Promise((resolve) => {
    try {
      ...
      let child;
      // Lazy load cross-spawn to avoid blocking main thread on startup
...
`;
    ...
    if (install.trim().toLowerCase() !== 'n') {
      ...
      try {
        // Lazy load cross-spawn for the installation step
